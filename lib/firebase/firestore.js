...
export async function createUserDocument(user, name, extra = {}) {
  ...
  const userData = {
    ...
    brNumber: extra.brNumber || '',
    organizationAddress: extra.organizationAddress || '',
...
export async function confirmPayment(orderId, paymentReference, adminId = null) {
  ...
  if (order.status === 'payment_confirmed') {
    const existing = await getDocs(query(collection(getDb(), 'services'), where('orderId', '==', orderId)));
    if (!existing.empty) {
      return {
        invoiceId: null,
    ...
    return { invoiceId: null, serviceIds, recovered: true };
  }
