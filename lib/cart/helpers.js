...
export function validateCartItem(item) {
  ...
  if (item.type === 'live_streaming' && !item.packageId) return false;
  if (item.type === 'ssl_certificate' && !item.packageId) return false;
...
export function isAnnualOnlyItem(item) {
  return item?.annualOnly === true || item?.type === 'ssl_certificate';
}

export function getItemLinePrice(item, billingCycle = 'monthly') {
  const qty = item.quantity || 1;
  let price = item.price || 0;
  if (isAnnualOnlyItem(item)) {
    return price * qty;
  }
...
export function calculateCartSubtotal(items, billingCycle = 'monthly') {
  return items.reduce((sum, item) => sum + getItemLinePrice(item, billingCycle), 0);
}
...
export function createStreamingCartItem(pkg, options = {}) {
  return {
    id: generateCartItemId(),
    type: 'live_streaming',
    packageId: pkg.id,
    ...
    price: pkg.monthlyPrice,
    renewalPrice: pkg.renewalPrice,
    taxIncluded: pkg.taxIncluded ?? false,
    quantity: 1,
    billingCycle: 'monthly',
    config: {
      region: options.region || 'us-east',
      payAsYouGo: options.payAsYouGo ?? false,
      maxListeners: pkg.maxListeners,
      maxStreams: pkg.maxStreams,
      stack: 'nodejs-icecast-liquidsoap',
    },
  };
}

export function hasStreamingInCart(items = []) {
  return items.some((item) => item.type === 'live_streaming');
}

export function hasSslInCart(items = []) {
  return items.some((item) => item.type === 'ssl_certificate');
}

export function createSslCartItem(pkg, options = {}) {
  return {
    id: generateCartItemId(),
    type: 'ssl_certificate',
    packageId: pkg.id,
    name: `${pkg.name} (Annual)`,
    price: pkg.annualPrice,
    renewalPrice: pkg.renewalPrice,
    annualOnly: true,
    quantity: 1,
    billingCycle: 'annual',
    config: {
      ...
      validationType: pkg.validationType,
      wildcard: pkg.wildcard,
      tempSslIncluded: pkg.tempSslIncluded !== false,
      includesCdn: pkg.includesCdn || false,
      cdnPlanId: pkg.cdnPlanId || null,
    },
  };
}
