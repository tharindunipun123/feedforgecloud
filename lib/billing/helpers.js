...
/** Flat tax added at checkout for each package/line item in the cart. */
export const CHECKOUT_TAX_PER_PACKAGE = 3.5;
...
export function calculateCartTax(items = [], taxPerPackage = CHECKOUT_TAX_PER_PACKAGE) {
  return items.reduce((sum, item) => sum + taxPerPackage * (item.quantity || 1), 0);
}
