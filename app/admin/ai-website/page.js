...
export default function AdminAiWebsitePage() {
  ...
  useEffect(() => {
    getAllServices().then((all) => setServices(all.filter((s) => s.type === 'ai-website'))).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div>
      <PageHeader title="AI Website Orders" description="Manage AI website builder orders." />
      {services.length === 0 ? (
        <EmptyState title="No AI website orders" />
      ) : (
